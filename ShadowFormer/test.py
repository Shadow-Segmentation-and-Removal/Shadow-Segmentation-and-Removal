...
parser.add_argument('--input_dir', default='/teamspace/studios/this_studio/SAM-Adapter-PyTorch/ISTD_Dataset/test',
...
with torch.no_grad():
    ...
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        if args.cal_metrics:
            ...
            ssim_val_rgb.append(ssim_loss(gray_restored, gray_gt, data_range=1.0, channel_axis=None))
            ssim_val_ns.append(ssim_loss(gray_restored * (1 - bm.squeeze()), gray_gt * (1 - bm.squeeze()), data_range=1.0, channel_axis=None))
            ssim_val_s.append(ssim_loss(gray_restored * bm.squeeze(), gray_gt * bm.squeeze(), data_range=1.0, channel_axis=None))
